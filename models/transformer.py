...
class Transformer_vis(nn.Module):
    ...
    def forward(self, src, mask, pos_embed):
        ...
        mask = mask.flatten(1).bool()                         # (B, HW), True = padding
        # (tuỳ chọn) đảm bảo dtype/device khớp:
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed):
        ...
        src = src.permute(2, 0, 1)                 # (L, B, C)
        pos_embed = pos_embed.permute(1, 0, 2)     # (L, B, C)
        mask = mask.bool()                         # (B, L), True = padding
        # (tuỳ chọn) đảm bảo dtype/device khớp:
        pos_embed = pos_embed.to(dtype=src.dtype, device=src.device)
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)
